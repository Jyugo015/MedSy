...
CORS(translator_bp, resources={r"/*": {"origins": "*"}})
...
class ConversationContext:
    ...
    def add_message(self, session_id: str, role: str, content: str):
        ...
        self.history[session_id].translator_bpend({"role": role, "content": content})
...
@translator_bp.route('/favicon.ico')
def favicon():
    return send_from_directory(
        os.path.join(translator_bp.root_path, 'static'),
        'favicon.ico',
...
def translate_with_llm(text: str, source_lang: str, target_lang: str, session_id: str = "") -> str:
    ...
    #     "Content-Type": "translator_bplication/json"
    # }
    # payload = {
    #     "model": "gpt-3.5-turbo",
...
@translator_bp.route('/')
def index():
    return render_template('translator_index.html')
...
def generate_audio(text: str, lang: str) -> Optional[str]:
    try:
        ...
        # Language mtranslator_bping with proper gTTS codes
        lang_map = {
            'ms': 'ms',       # Malay
            'zh': 'zh',       # Chinese
...
@translator_bp.route('/generate_audio', methods=['POST'])
...
@translator_bp.route('/process_audio', methods=['POST'])
def handle_audio():
    try:
        ...
        return jsonify({'error': str(e)}), 500
@translator_bp.route('/audio/<filename>')
...
if __name__ == '__main__':
    translator_bp.run(host='0.0.0.0', port=5000, threaded=True)